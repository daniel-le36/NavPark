  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function(position) {
      ...
      var onClickHandler = function() {
        ...
        var parkingLatitude = results[0].geometry.location.lat();
        var parkingLongitude = results[0].geometry.location.lng();
        ...
        var goPromise = new Promise(function(resolve,reject){
          $.ajax({
            url : "/get_parking?latitude="+parkingLatitude+"&longitude="+parkingLongitude,
            method: 'GET',
        ...
        goPromise.then(function(data){
          ...
          calculateWalkingRoute(directionsService2, directionsRenderer2, map, markerWalkHere, parkingCoords);
        });
      })
      };
      ...
      var onClickHandler = function() { //when the Go button is clicked
        ...
        var parkingLongitude = results[0].geometry.location.lng();
            ...
            if (status == google.maps.GeocoderStatus.OK) { //if geocoder working
              ...
              goPromise.then(function(data){
                parkingCoords = new google.maps.LatLng(data["Parking_Coords"][0],data["Parking_Coords"][1]);
                calculateDrivingRoute(directionsService, directionsRenderer, map, markerDriveHere, myLatLng, parkingCoords);
                calculateWalkingRoute(directionsService2, directionsRenderer2, map, markerWalkHere, parkingCoords);
              });
            }
  ...
  geocoder.geocode({'address': document.getElementById('inputEnd').value}, function(results, status) { //replace 'end' with parking's location
    if (status == google.maps.GeocoderStatus.OK) { //if geocoder working
      ...
      directionsService.route( //generate route using input from user and external API's coordinates
          {
            origin: pos,
            destination: parkingSpot, // USE EXTERNAL API'S LONGITUDE AND LATITUDE
            travelMode: 'DRIVING'
          },
